...
export default function JustifyCenterTest() {
  const boxes = D.range(0, 5).map(index => (
    <Div key={index} className="w-4 h-4  bg-black" minHeight="auto" />
  ))
  ...
  return (
    <section className="p-4 mt-4 border-2">
      ...
      <div className="mt-4 border-2">
        ...
        <div className="flex flex-row items-center h-40 bg-gray-300 justify-between min-h-40">
          {boxes}
        </div>
      </div>
      <div className="mt-4">
        ...
        <div className="flex flex-col items-center justify-evenly h-40 bg-gray-300">
          {boxes}
        </div>
      </div>
    </section>
